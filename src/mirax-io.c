...
char *concat_wildcard_string_int32(const char *str, int32_t integer) {
    char *result_string = (char *)malloc(strlen(str) + number_of_digits(integer) + 1);
    sprintf(result_string, str, integer);
    return result_string;
}

char *concat_wildcard_string_m_int32(const char *str, int32_t integer1, int32_t integer2) {
...
struct mirax_file *get_mirax_layers(struct ini_file *ini, int32_t l_count) {
    ...
    for(int32_t layer_id = 0; layer_id < l_count; layer_id++) {
        ...
        char *nonhier_level_count_key = concat_wildcard_string_int32(NONHIER_X_COUNT, layer_id);
        char *nonhier_level_count = get_value_from_ini_file(ini, HIERARCHICAL, nonhier_level_count_key);
        int32_t nhl_count;
        sscanf(nonhier_level_count, "%d", &nhl_count);
        layer->level_count = nhl_count;
        char *layer_name_key = concat_wildcard_string_int32(NONHIER_X_NAME, layer_id);
        ...
        for(int32_t level_id = 0; level_id < nhl_count; level_id++) {
            ...
            level->record = layer_id + level_id; // correct?
            level->key_prefix = concat_wildcard_string_m_int32(NONHIER_X_VAL_X, layer_id, level_id);
            level->name = get_value_from_ini_file(ini, HIERARCHICAL, level->key_prefix);
            char *section_key = (char *)malloc(sizeof(level->key_prefix) + sizeof(_SECTION) + 1);
...
int32_t *read_data_location(char *filename, int32_t record, int32_t **position, int32_t **size) {
    FILE *fp = fopen(filename, "r");

    if(fp == NULL) {
        printf("Error: Could not open file stream.\n");
        return NULL;
    }
    
    if(fseek(fp, MRXS_ROOT_OFFSET_NONHIER, SEEK_SET)) {
        printf("Error: Can not seek to mrxs offset in Index.dat.\n");
        fclose(fp);
        return NULL;
    }
    ...
    // read in the file number, offset and length of the image strip
...
int32_t zero_level(char *filename, int32_t **offset, int32_t **length, const char *prefix) {
    ...
    if(fseek(fp, 0, SEEK_END)) {
        printf("Error: Failed reading to end of file.\n");
        fclose(fp);
        return 1;
    }
    ...
    if(truncation) {
        // TODO
        // replace with truncation (must work under win and linux!)
        fseek(fp, **offset, SEEK_SET);
        char *empty_buffer = get_empty_char_buffer("\0", **length);
...
int32_t delete_level(char *path,
    ...
    if(level_to_delete == NULL) {
        printf("Error: Could not find slide barcode level");
        return -1;
    }
    ...
    return zero_level(filename, &position, &size, "\xff\xd8\0");
}

int32_t delete_record_from_index_file(char *filename, int32_t record, int32_t all_records) {
    FILE *fp = fopen(filename, "r+w");
    ...
    if(fread(buffer, (to_move * 4), 1, fp) != 1) {
        printf("Error: Unexpected length of buffer.\n");
        fclose(fp);
        return -1;
    }

    // we return to record position and overwrite the file with the tail data
    fseek(fp, (*table_base + (record * 4)), SEEK_SET);
...
int32 handle_mirax(const char *filename, const char *new_label_name, bool unlink_directory) {
    ...
    if(index_filename == NULL 
        || file_count == NULL 
        || layer_count == NULL) {
        printf("Error: No index file specified.\n");
        return -1;
    }
    ...
    for(int32_t i = 0; i < f_count; i++) {
        ...
        char *temp_datafile_name = get_value_from_ini_file(ini, DATAFILE, temp_datafile_key); 
        data_filenames[i] = temp_datafile_name;
        //printf("%i: %s\n", i, temp_datafile_name);
    }

    struct mirax_file *mirax_file = get_mirax_layers(ini, l_count);

    // wipe the image data in the data file
    int32_t result = delete_level(path, index_filename, data_filenames, mirax_file->layers, SCAN_DATA_LAYER, SLIDE_BARCODE);

    if(result != 0) {
        return result;
    }

    struct mirax_level *level_to_delete = get_level_by_name(mirax_file->layers, SCAN_DATA_LAYER, SLIDE_BARCODE);
    ...
    while(next_level != NULL) {
        // we determine the predecessor level of the deleted level and change non hier entries
    ...
    char *nonhier_plain = concat_wildcard_string_m_int32(NONHIER_X_VAL_X, 0, level_to_delete->id);
    char *nonhier_section = concat_wildcard_string_m_int32(NONHIER_X_VAL_X_SECTION, 0, level_to_delete->id);
    char *nonhier_imgx = concat_wildcard_string_m_int32(NONHIER_X_VAL_X_IMAGENUMBER_X, 0, level_to_delete->id);
    char *nonhier_imgy = concat_wildcard_string_m_int32(NONHIER_X_VAL_X_IMAGENUMBER_Y, 0, level_to_delete->id);
