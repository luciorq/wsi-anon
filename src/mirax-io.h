...
int32_t handle_mirax(const char **filename, const char *new_label_name, bool keep_macro_image, bool disable_unlinking, bool do_inplace);
...
int32_t wipe_level_data(const char *filename, int32_t **offset, int32_t **length, const char *prefix, const char *suffix);

int32_t delete_level(const char *path, const char *index_file, const char **data_files, struct mirax_layer **layers, const char *layer_name, const char *level_name);
...
int32_t replace_slide_id_in_indexdat(const char *path, const char *filename, const char *value, const char *replacement, int32_t size);

int32_t replace_slide_id_in_datfiles(const char *path, const char **data_files, int32_t length, const char *value, const char *replacement, int32_t size);

void remove_metadata_in_data_dat(const char *path, const char **data_files, int32_t length, const char *var);

int32_t wipe_data_in_index_file(const char *path, const char *index_filename, struct mirax_level *level_to_delete, struct mirax_file *mirax_file);

int32_t wipe_delete_unlink(const char *path, struct ini_file *ini, const char *index_filename, struct mirax_file *mirax_file, const char *layer, const char *level_to_delete);
